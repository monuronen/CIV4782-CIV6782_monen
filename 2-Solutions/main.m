...
%% Create a table for all calculated indicators and save it in an xls file
Metric = {'Failure count'; 'Reliability'; ...
    'Resilience'; 'Vulnerability'; 'Percentage vulnerability'; ...
    'Volumetric reliability'};
Chester_Demand = aggregate_demand(indicators.chester);
Baltimore_Demand = aggregate_demand(indicators.baltimore);
Peach_Bottom = aggregate_demand(indicators.peach_bottom);
Ecological_Releases = aggregate_demand(indicators.ecological);
Flooding = aggregate_other(indicators.flooding);
Recreation = aggregate_other(indicators.recreation);
Tbl = table(Metric,Chester_Demand,Baltimore_Demand,...
    Peach_Bottom,Ecological_Releases,Flooding,Recreation);
disp(Tbl)
writetable(Tbl,'Indicators.xlsx','Sheet','Results','Range','A1:G7')
